...
# ---------------------------------------------------------
# Load Model Artifact (With Cloud Compatibility Fallback)
# ---------------------------------------------------------
@st.cache_resource
def load_model_artifact(model_path="smart_home_appliance_energy_model.pkl", data_path="energydata_complete.csv"):
    # 1. Attempt loading existing pickled artifact
    if os.path.exists(model_path):
        try:
            ...
            st.warning(f"Pickle version mismatch detected ({err}). Automatically compiling compatible model for current environment...")

    # 2. Auto-compile fallback model if pickle load fails or file is missing
    if os.path.exists(data_path):
        ...
        st.error(f"Neither model artifact `{model_path}` nor dataset `{data_path}` was found.")
        return None
