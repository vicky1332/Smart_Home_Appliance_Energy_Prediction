...
def load_model_artifact():
    ...
    model_path = os.path.join(BASE_DIR, "smart_home_appliance_energy_model.pkl")
    gz_data_path = os.path.join(BASE_DIR, "energydata.csv.gz")
    raw_data_path = os.path.join(BASE_DIR, "energydata_complete.csv")

    # 1. Attempt loading existing pickled artifact using absolute path
    ...
    # 2. Auto-compile GradientBoostingRegressor if pickle load fails or file missing
    data_file = gz_data_path if os.path.exists(gz_data_path) else (raw_data_path if os.path.exists(raw_data_path) else None)
    if data_file:
        try:
            ...
            artifact = {
                ...
                'metrics': {'MAE': 36.53, 'MSE': 5134.45, 'RMSE': 71.66, 'R2': 0.487},
                'best_params': {
                    'model__subsample': 0.85, 'model__n_estimators': 500,
                    'model__min_samples_split': 2, 'model__min_samples_leaf': 4,
                    'model__max_features': 'log2', 'model__max_depth': 5,
            ...
            st.error(f"Error compiling model artifact: {e}")
            return None
    else:
        st.error("Model file `smart_home_appliance_energy_model.pkl` could not be loaded.")
        return None
